...
> As of [Nuxt@2.14](https://github.com/nuxt/nuxt/releases/tag/v2.14.0) Nuxt automatically detects the current version of `core-js` in your project, also you can specify which version you want to use.
...
The default targets of [@nuxt/babel-preset-app](https://github.com/nuxt/nuxt/blob/2.x-dev/packages/babel-preset-app/src/index.js) are `ie: '9'` in the `client` build, and `node: 'current'` in the `server` build.
...
- Argument:
  ...
  2. `Array`:
     ...
     - [`options`](https://github.com/nuxt/nuxt/tree/2.x-dev/packages/babel-preset-app#options) of `@nuxt/babel-preset-app`
...
If you want to see more about our default webpack configuration, take a look at our [webpack directory](https://github.com/nuxt/nuxt/tree/2.x-dev/packages/webpack/src/config).
...
This mode bundles `node_modules` that are normally preserved as externals in the server build ([more information](https://github.com/nuxt/nuxt/pull/4661)).
