...
> Depuis [Nuxt@2.14](https://github.com/nuxt/nuxt.js/releases/tag/v2.14.0), Nuxt va automatiquement détecter la version actuelle de `core-js` dans notre projet. On peut bien sûr aussi lui spécifier la version que l'on veut utiliser.
...
Ce mode regroupe des `node_modules` qui sont normalement conservés en tant qu'externes dans la construction du serveur ([plus d'informations](https://github.com/nuxt/nuxt.js/pull/4661)).
